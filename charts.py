...
def chart_static_weather_counts(df: pd.DataFrame) -> alt.Chart:
    """STATIC: counts of each weather category."""
    return (
        alt.Chart(df)
        .mark_bar()
        .encode(
            x=alt.X("weather:N", title="Weather type", sort="-y"),
            y=alt.Y("count():Q", title="Days"),
            tooltip=[alt.Tooltip("weather:N"), alt.Tooltip("count():Q", title="Days")],
        )
        .properties(height=320)
    )


def chart_interactive_monthly_temp(df: pd.DataFrame) -> alt.Chart:
    """INTERACTIVE: brush months; linked histogram updates."""
    ...
    top = (
        ...
        .encode(
            x=alt.X("date:T", title="Date"),
            y=alt.Y("temp_max:Q", title="Daily max temp (°C)"),
            tooltip=[alt.Tooltip("date:T"), alt.Tooltip("temp_max:Q", format=".1f"), "weather:N"],
    ...
    return alt.vconcat(top, bottom).properties(spacing=10)
